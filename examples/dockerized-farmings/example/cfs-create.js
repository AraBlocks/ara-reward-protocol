...
async function createFarmerCFS(filePath, cfsPath, jsonPath) {
  ...
  await fs.writeFileSync(jsonPath, `{"id": "${cfsId}", "key": "${cfs.key.toString('hex')}"}`, function(err) {
      if(err) {
        return console.log(err);
      }
      console.log('json was saved in the same folder');
  })
...
async function mirrorPath(filePath, cfs) {
  ...
  // Mirror and log
  const progress = mirror({ name: filePath }, { name, fs: cfs }, { keepExisting: true })
