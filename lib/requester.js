...
class Requester {
  ...
  /**
   * On receipt of a signed (and staked) contract from farmer, deploys job with budget to contract and can begin distribution of work.
   * @param {Error} err
  ...
  handleSignedContract(err, response) {
    if (err) {
      ...
      this.matcher.onHireConfirmed(response, function() {
        this.contractGen.deployJobToContract();
      });
    } else {
  ...
  /**
   * After a job is finished, submits reward for each farmer to contract and notifies them that their reward is ready to be withdrawn
   */
  sendReward() {
    const contractGen = this.contractGen;
    const sow = this.sow;
    this.farmers.forEach(farmer => {
      farmer.getQuote(this.sow, function(error, quote) {
        const reward = quote.getPerUnitCost() * 3;
        contractGen.submitRewardToContract(farmer.getId, sow.getId(), reward);
      });
