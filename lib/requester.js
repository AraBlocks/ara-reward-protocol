...
class Requester {
  ...
  sendReward() {
    Object.keys(this.matcher.hiredWorkerQuoteOptions).forEach(function(key) {
      // Calculate reward for each farmer
    });

    // Build a Merkle tree with each reward as leaf and send it to the contract
    // Notify farmers to withdraw their rewards with index, address, and rate
  }
}
