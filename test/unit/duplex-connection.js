...
test('duplex.encode-decode.validData', (t) => {
  ...
  const sow = new SOW()
  sow.setNonce(bb)
  const encodedSow = MSG.encode(MSG.SOW.type, sow.serializeBinary())
  ...
  quote.setNonce(bb)
  const encodedQuote = MSG.encode(MSG.QUOTE.type, quote.serializeBinary())
  ...
  const encodedAgreement = MSG.encode(MSG.AGREEMENT.type, agreeement.serializeBinary())
  ...
  reward.setNonce(bb)
  const encodedReward = MSG.encode(MSG.REWARD.type, reward.serializeBinary())
  ...
  receipt.setNonce(bb)
  const encodedReceipt = MSG.encode(MSG.RECEIPT.type, receipt.serializeBinary())
